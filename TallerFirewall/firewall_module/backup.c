...
static unsigned char In_policy; /* Default policy for inbound list; 1 = Allow, 2 = Deny */
struct list_head Out_lhead; /* Head of outbound-rule list */
static unsigned char Out_policy; /* Default policy for outbound list ; 1 = Allow, 2 = Deny */

static int Device_open; /* Opening counter of a device file */
static char *Buffer;    /* A buffer for receving data from a user space */

static bool check_ip(unsigned int ip, unsigned int rule_ip, unsigned int rule_mask) {
    if (NOT_ZERO(rule_mask)) {
        return EQUAL_NET_ADDR_MASK(ip, rule_ip, rule_mask);
    } else {
        return EQUAL_NET_ADDR(ip, rule_ip);
    }
}
 ...
 */
static unsigned int mfw_general_filter(void *priv, struct sk_buff *skb,
    ...
    if (!skb || rule_list_head->next == rule_list_head) return NF_ACCEPT;
    ...
    list_for_each_entry(node, listh, list) {
        ...
        if (NOT_ZERO(r->proto) && (r->proto != iph->protocol))
            continue;

        if (NOT_ZERO(r->s_ip) && !check_ip(s_ip, r->s_ip, r->s_mask))
            continue;

        if (NOT_ZERO(r->s_port) && (r->s_port != s_port)) 
            continue;

        if (NOT_ZERO(r->d_ip) && !check_ip(d_ip, r->d_ip, r->d_mask)) 
            continue;

        if (NOT_ZERO(r->d_port) && (r->d_port != d_port))
            continue;
 ...
 */
static unsigned int mfw_in_filter(void *priv, struct sk_buff *skb,
                                  const struct nf_hook_state *state) {
    return NF_DROP;// mfw_general_filter(priv, skb, state, &In_lhead);
}

/*
 * Outbound filter is applied to all outbound packets.
 */
static unsigned int mfw_out_filter(void *priv, struct sk_buff *skb,
                                   const struct nf_hook_state *state) {
 ...
 */
static int mfw_dev_open(struct inode *inode, struct file *file) {
 ...
 */
static int mfw_dev_release(struct inode *inode, struct file *file) {
 ...
 * until it returns 0.
 */
static ssize_t mfw_dev_read(struct file *file, char *buffer, size_t length,
                            loff_t *offset) {
    int byte_read = 0;
 ...
 */
static void mfw_rule_add(struct mfw_rule *rule) {
    ...
    printk(KERN_INFO "src %d.%d.%d.%d : %d   dst %d.%d.%d.%d : %d   proto %d\n",
 ...
 */
static void mfw_rule_del(struct mfw_rule *rule) {
    struct rule_node *node;
 ...
 */
static ssize_t mfw_dev_write(struct file *file, const char *buffer,
...
static int __init mfw_mod_init(void) {
    ...
    if (Buffer == NULL) {
        printk(KERN_ALERT "MiniFirewall: Fails to start due to out of memory\n");
        return -1;
    }

    INIT_LIST_HEAD(&In_lhead);
    INIT_LIST_HEAD(&Out_lhead);
    /* Start allow default policy because there are no rules define */
    In_policy = 1;
    Out_policy = 1;
